// window.baseIp1 = "http://10.112.143.192"

window.baseIp1 = "http://10.112.143.191"
window.baseIp2 = "http://10.112.143.191"
// window.baseIp1 = "http://222.212.82.225"
// window.baseIp2 = "http://222.212.82.225"
window.baseRequestUrl = window.baseIp1 + ":20128/api"; // 后端接口地址
window.baseDeploymentAddress = window.baseIp1 + ":8003";
window.zhddUrl = window.baseIp1 + ":20128/map/index_dispatch"; // 指挥调度跳转地址
window.baseWebrtcUrl = window.baseIp2 + ":8078"; // 视频监控的地址


window.kdApiKey = "b264c6096dc545a5a7ea9ec1c47c333f"

window.STORE_INFO = {}
