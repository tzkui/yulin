<template>
    <!-- 下面这里再来测试一下这个弹出框的东西了 -->
        <viewDialog  :style="{ top: top, left: left }"  class="tcbox" :title="title" :showDialog="islook" @closeDialog="closeDialog">
            <div>
                <!-- <div class="header">
            ...
            <img class="buttom" src="../../assets/emergencybomb/dibu.png" alt="">
                ...
                <div class="main">
                    <div class="mainitem" v-for="item in data">
                        <span>{{ item.name }}:</span>
                        <!-- 这里还可以根据判断这个状态的颜色字体的 -->
                        <p :class="item.name == '状态'? 'cochange' : ''">{{ item.content }} <img v-if="item.img" :src="item.img" alt=""> </p>
                    </div>
                </div>
                <!-- 下面还有底部的按钮功能的 -->
                <div class="buttons">
                    ...
                    <div class="btns">
                        <button v-for="(item, index) in buttondata" @click="onAdd(index)">{{ item }}</button>
                    </div>
                </div>
            </div>
        </viewDialog>
...
//  使用传过来的值
const props = defineProps(["title", "data", "buttondata", "imgs", "islook", "top","left"]);
