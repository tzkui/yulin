...
const filterMethod = (value, data) => {
  ...
  return data.label?.includes(value) || data.phone?.linkPhone(value);
};
</script>
<template>
  <firDialog :name="name" @closeDialog="closeDialog">
    <div class="search_box" v-if="name==='通讯录'">
      <el-input 
        placeholder="搜索人名或电话" 
        v-model="searchVal" 
        @keyup.enter="searchTxl"
        @clear="searchTxl"
        clearable
      >
      <template #append>
      ...
      >
        <template #default="{ node, data }">
          <span class="custom-tree-node">
            <span v-if="data.dataType === 2&&name==='通讯录'">
              <img class="online_status" v-if="data.appdlzt==1" src="@/assets/home/online.png" />
              <img class="online_status" v-else src="@/assets/home/offline.png" />
            </span>
